...
def htuning(*, model_class: LightningModule, hparam_objective: Callable[[optuna.trial.Trial], Sequence[float]], datamodule: LightningDataModule, 
            valid_metrics: Sequence[str], datamodule_kwargs: Dict[str, Any] = dict(pin_memory = True, num_workers = 8, batch_size = 6 ),
            directions: Sequence[str] | str, precision: str,n_trials: int, 
    ...
    def objective(trial: optuna.trial.Trial):
        ...
        return (
            trainer.callback_metrics[valid_metric].item() for valid_metric in valid_metrics    
        )
    ...
    if isinstance(valid_metrics, str):
        ...
        study.optimize(objective, n_trials = n_trials, gc_after_trial=True, show_progress_bar=True, n_jobs = -1) 
