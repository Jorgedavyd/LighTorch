...
class DeepNeuralNetwork(nn.Sequential):
    ...
    """
    def __init__(self, in_features: int, layers: Sequence[int], activations: Sequence[nn.Module, None]):
        assert (len(layers) == len(activations)), 'Must have the same amount of layers and activation functions'
