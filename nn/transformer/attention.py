...
class CrossAttention(nn.Module):
    def __init__(self, type: _AttentionBase, method: str = 'i i c', *args, **kwargs) -> None:
        super().__init__()
        self.attention = type(*args, **kwargs)
...
class GroupedQueryAttention(_AttentionBase):
    def __init__(self, embed_dim: int, n_queries: int, n_groups: int, kdim: int = None, vdim: int = None, **kwargs) -> None:
        ...
        assert (vdim%n_queries == 0), 'Not valid number of heads, not divisible by embedding dimension or values dimension'
        assert (n_queries%n_groups == 0), 'The number of query heads should be divisible by the number of groups'
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, mask: Tensor = None) -> Tensor:
        ...
        q = rearrange(self.Wq(q), 'b s (g q d) -> b g q s d', q = self.query_heads, g = self.n_groups) # -> (B, S, vdim) -> (B, n_groups, n_queries, S, query_dim)
        k = rearrange(self.Wk(k), 'b s (g d) -> b g s d', g = self.n_groups) # -> (B, n_groups, S, query_dim)
        v = rearrange(self.Wv(v), 'b s (g d) -> b g s d', g = self.n_groups) # -> (B, n_groups, S, query_dim)
...
class MultiHeadAttention(_AttentionBase):
    def __init__(self, embed_dim: int, n_heads: int, kdim: int = None, vdim: int = None, **kwargs) -> None:
        ...
        assert (vdim%n_heads == 0), 'Not valid number of heads, not divisible by embedding dimension or values dimension'
...
class MultiQueryAttention(_AttentionBase):
    def __init__(self, embed_dim: int, n_queries: int, kdim: int = None, vdim: int = None, **kwargs) -> None:
        super(MultiQueryAttention, self).__init__(**kwargs)
        if vdim is None:
            vdim = embed_dim
        if kdim is None:
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, mask: Tensor = None) -> Tensor:
        ...
        # Reshaping for broadcasting
        q = self.Wq(q).view(*q_shape).transpose(1, 2) # -> (B, s, embed_dim) -> (B, query_heads, S, query_dim)
...
__all__ = ['MultiQueryAttention', 'MultiHeadAttention', 'GroupedQueryAttention']